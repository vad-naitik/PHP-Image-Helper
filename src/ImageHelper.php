...
class ImageResize
{
     ...
     * @return ImageResize
     * @throws ImageHelperException
     */
    public static function createFromString($image_data)
    {
        if (empty($image_data) || $image_data === null) {
            throw new ImageHelperException('image_data must not be empty');
        }
    ...
    public function __construct($filename)
    {
        ...
        if ($filename === null || empty($filename) || (substr($filename, 0, 5) !== 'data:' && !is_file($filename))) {
            throw new ImageHelperException('File does not exist');
        }
        ...
        if (strstr(finfo_file($finfo, $filename), 'image') === false) {
            if (version_compare(PHP_VERSION, '7.0.0', '<=') && strstr(file_get_contents($filename), 'WEBPVP8') !== false) {
                ...
                throw new ImageHelperException('Unsupported file type');
            }
        ...
        if (!$checkWebp) {
            if (!$image_info) {
                if (strstr(finfo_file($finfo, $filename), 'image') !== false) {
                    throw new ImageHelperException('Unsupported image type');
                }

                throw new ImageHelperException('Could not read file');
            }
        ...
        case IMAGETYPE_BMP:
            if (version_compare(PHP_VERSION, '7.2.0', '<')) {
                throw new ImageHelperException('For bmp support PHP >= 7.2.0 is required');
            }
        ...
        if (!$this->source_image) {
            throw new ImageHelperException('Could not load image');
        }
    ...
    public function save($filename, $image_type = null, $quality = null, $permissions = null, $exact_size = false)
    {
        ...
        case IMAGETYPE_WEBP:
            if (version_compare(PHP_VERSION, '5.5.0', '<')) {
                throw new ImageHelperException('For WebP support PHP >= 5.5.0 is required');
            }
            if( !empty($exact_size) && is_array($exact_size) ){
                $dest_image = imagecreatetruecolor($exact_size[0], $exact_size[1]);
                $background = imagecolorallocate($dest_image, 255, 255, 255);
        ...
        case IMAGETYPE_BMP:
            if (version_compare(PHP_VERSION, '7.2.0', '<')) {
                throw new ImageHelperException('For WebP support PHP >= 7.2.0 is required');
            }
